...
<body>
    ...
    <table class=middle>
        ...
        <?php
            ...
            $sql = "SELECT * FROM question_borde ORDER BY qb_no DESC limit $start, 10";
            $res = mysqli_query($conn, $sql);

            while($row = mysqli_fetch_array($res)){ ?>
            <tbody>
                <tr align=center>
                    <td><?php echo $row['qb_no'];?></td>
                    <td><a href="view.php?id=<?=$row['qb_no']?>"><?php echo $row['qb_title'];?></a></td>
                    <td><?php echo $row['qb_writer'];?></td>
                    <td><?php echo $row['qb_date'];?></td>
                    <td><?php echo $row['qb_hits'];?></td>
                </tr>
            </tbody>
        <?php } ?>
    </table>
    ...
    <?php
        // 페이징 링크 생성
        $sql = "SELECT COUNT(*) AS qb_no FROM question_borde";
        $res = mysqli_query($conn, $sql);
        $row = mysqli_fetch_assoc($res);
        $total_posts = $row['qb_no'];
