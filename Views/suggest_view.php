...
<body>
    ...
    <table class=middle>
        ...
        <?php
            ...
            $sql = "SELECT * FROM suggest_borde ORDER BY sb_no DESC limit $start, 10";
            $res = mysqli_query($conn, $sql);

            while($row = mysqli_fetch_array($res)){ ?>
            <tbody>
                <tr align=center>
                    <td><?php echo $row['sb_no'];?></td>
                    <td><a href="view.php?id=<?=$row['sb_no']?>"><?php echo $row['sb_title'];?></a></td>
                    <td><?php echo $row['sb_writer'];?></td>
                    <td><?php echo $row['sb_date'];?></td>
                    <td><?php echo $row['sb_hits'];?></td>
                </tr>
            </tbody>
        <?php } ?>
    </table>
    ...
    <?php
        // 페이징 링크 생성
        $sql = "SELECT COUNT(*) AS sb_no FROM suggest_borde";
        $res = mysqli_query($conn, $sql);
        $row = mysqli_fetch_assoc($res);
        $total_posts = $row['sb_no'];
