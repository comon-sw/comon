...
<body>
    ...
    <table class=middle>
        ...
        <?php
            ...
            $sql = "SELECT * FROM free_borde ORDER BY fb_no DESC limit $start, 10";
            $res = mysqli_query($conn, $sql);

            while($row = mysqli_fetch_array($res)){ ?>
            <tbody>
                <tr align=center>
                    <td><?php echo $row['fb_no'];?></td>
                    <td><a href="view.php?id=<?=$row['fb_no']?>"><?php echo $row['fb_title'];?></a></td>
                    <td><?php echo $row['fb_writer'];?></td>
                    <td><?php echo $row['fb_date'];?></td>
                    <td><?php echo $row['fb_hits'];?></td>
                </tr>
            </tbody>
        <?php } ?>
    </table>
    ...
    <?php
        // 페이징 링크 생성
        $sql = "SELECT COUNT(*) AS fb_no FROM free_borde";
        $res = mysqli_query($conn, $sql);
        $row = mysqli_fetch_assoc($res);
        $total_posts = $row['fb_no'];
